...
protected:
    ...
    int getTamanhoConjunto(int value) {
        return tam[buscar(value)];
    }
...
public:
    ...
    ConjuntosDisjuntos(int N) : ncjtos(N), p(N), tam(N,1), mayorConjunto(1) {
        for (int i = 0; i < N; ++i)
        p[i] = i;
    }
    ...
    void unir(int a, int b) {
        ...
        if (tam[i] > tam[j]) { // i es la raíz del árbol más grande
            tam[i] += tam[j]; p[j] = i;
            mayorConjunto = std::max(mayorConjunto, getTamanhoConjunto(b));
        } else {
            tam[j] += tam[i]; p[i] = j;
            mayorConjunto = std::max(mayorConjunto, getTamanhoConjunto(a));
        }
        --ncjtos;
    }
