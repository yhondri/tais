...
protected:
    ...
    struct TreeNode {
        ...
        size_t altura;
        size_t tam_i;
        TreeNode(clave_valor const& e, Link i = nullptr, Link d = nullptr, int alt = 1, int tam = 1) : cv(e), iz(i), dr(d), altura(alt), tam_i(tam){}
    };
...
public:
    ...
    void mostrarClaves(int minValue, int maxValue) {
        mostrarClaves(minValue, maxValue, raiz);
    }
    
    void mostrarClaves(int minValue, int maxValue, Link raiz) {
        if (raiz == NULL) {
            return;
        }
        
        if (minValue <= raiz->cv.clave) {
            mostrarClaves(minValue, maxValue, raiz->iz);
        }
        if (raiz->cv.clave >= minValue && raiz->cv.clave <= maxValue) {
            cout << raiz->cv.clave << " ";
        }
        if (raiz->cv.clave <= maxValue) {
            mostrarClaves(minValue, maxValue, raiz->dr);
        }
    }
    
    int buscarClave(size_t clave) {
        Link link = buscarClave(clave, raiz);
        if(link != nullptr) {
            return link->cv.clave;
        }
        return -1;
    }
    
    Link buscarClave(size_t positionElem, Link raiz) {
        if (raiz == NULL) {
            return nullptr;
        }
        else if (positionElem < raiz->tam_i) {
            return buscarClave(positionElem, raiz->iz);
        }
        else if (positionElem > raiz->tam_i) {
            return buscarClave(positionElem - raiz->tam_i, raiz->dr);
        }
        else {
            return raiz;
        }
    ...
    void mostrarArbol(Link raiz) {
        if (raiz == NULL) {
            return;
        }
        cout << "Raíz " << raiz->cv.clave << " " << raiz->tam_i;
        
        if (raiz->iz != nullptr) {
            cout << " izquierdo " << raiz->iz->cv.clave<< " " ;
        }
        
        if (raiz->dr != nullptr) {
            cout << " Derecho " << raiz->dr->cv.clave;
        }
        cout << " " << endl;
        if (raiz->iz != nullptr) {
            mostrarArbol(raiz->iz);
        }
        if (raiz->dr != nullptr) {
            mostrarArbol(raiz->dr);
        }
    }
...
protected:
    ...
    int calcularTamI_aux(Link & a) {
        if (a == nullptr) {
            return 0;
        }
        
        int hijosIz = calcularTamI_aux(a->iz) + 1;
        a->tam_i = hijosIz;
        
        return hijosIz + calcularTamI_aux(a->dr);
    }
    ...
    static int actualizarTamanho(Link & raiz) {
        if (raiz == nullptr) {
            return 0;
        }
        
        int hijos = actualizarTamanho(raiz->iz) + 1;
        
        raiz->tam_i = hijos;
        
        return hijos + actualizarTamanho(raiz->dr);
    }
